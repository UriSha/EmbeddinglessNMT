...
def save_checkpoint(args, trainer, epoch_itr, val_loss):
    ...
    if val_loss is not None and args.keep_best_checkpoints > 0:
        name_template = "checkpoint.best_{}_{:." + str(args.precision) + "f}.pt"
        checkpoint_conds[name_template.format(args.best_checkpoint_metric, val_loss)] = True
    ...
    if args.keep_best_checkpoints > 0:
        ...
        if not args.maximize_best_checkpoint_metric:
            checkpoints = sorted(checkpoints, key=lambda s: float(s[s.rfind('_')+1:-3]))
